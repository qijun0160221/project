...
public class ConnDB {
	public Connection conn = null; // 声明Connection数据库连接对象
	public Statement stmt = null; // 声明Statement执行sql对象
	public ResultSet rs = null; // 声明ResultSet结果集对象
	...
	public static Connection getConnection() {
		...
		try { // 连接数据库时可能发生异常因此需要捕捉该异常
			System.out.println("执行连接语句getConnection");
	...
	public ResultSet executeQuery(String sql) {
		try { // 捕捉异常
			System.out.println("执行连接语句executeQuery");
			conn = DriverManager.getConnection(dbUrl); // 调用getConnection()方法构造Connection对象的一个实例conn
	...
	public int executeUpdate(String sql) {
		...
		try { // 捕捉异常
			System.out.println("执行连接语句executeupdate");
